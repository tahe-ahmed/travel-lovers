...
const BucketMap = (props) => {
  ...
  const [userLocation, setUserLocation] = useState([props.longitude, props.latitude]);

  mapboxgl.accessToken = process.env.REACT_APP_MAPBOX_ACCESS_TOKEN;
  //Longitude: 7.0262 | Latitude: 43.8411 | Zoom: 0.50
  useEffect(() => {
    setMapPlaces(props.placesData);
    if (Number.isNaN(userLocation[0]) || typeof userLocation[1] == 'undefined') {
      setUserLocation([props.longitude, props.latitude]);
    } else {
      if (mapPlaces) {
